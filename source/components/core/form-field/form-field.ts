import { AbstractFormField } from "~/library/scripts/WpForm/AbstractFormField";
import { phoneModifier } from "~/library/scripts/WpForm/inputModifiers/formFieldModifiers";
import { FormFieldState } from "~/library/scripts/WpForm/FormFieldState";
import { emailValidator, phoneValidator } from "~/library/scripts/WpForm/validators/formFieldValidators";
...
export default class FormField extends AbstractFormField<string | string[] | FileList | null> {
    ...
    private createFileFieldState(
        ...
        const fileNameEl = this.element.querySelector<HTMLElement>("[class*='__file-name']");
        control.addEventListener("change", () => {
            fieldState.value = control.files;
            if (fileNameEl) {
                const names = control.files ? Array.from(control.files).map(f => f.name) : [];
